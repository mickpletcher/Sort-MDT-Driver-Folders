...
<#
	.PARAMETER DeploymentShare
		Root directory of the deployment share
...
#Add backslash to end of $DeploymentShare if it does not exist
If ($DeploymentShare.Substring($DeploymentShare.Length - 1) -ne '\') {
	$DeploymentShare += '\'
}

$FileToEdit = $DeploymentShare + 'Control\DriverGroups.xml'

#Set the export path where you want the new file to go.
$exportPath = $DeploymentShare + 'Control\DriverGroups.xml'
...
$BackupFolderPath = $DeploymentShare + 'Control\DriverGroups\'

#Exits the program if it is unable to read/find the file.
Try {
	[xml]$xml = get-content $FileToEdit -ErrorAction Stop
} Catch {
...
If ($BackupFile.IsPresent) {
	...
	If (Test-Path -Path $BackupFolderPath){
		...
		Try {
			...
			Write-Host "Could not access path $BackupFolderPath. Check your path/access rights." -ForegroundColor Yellow}
	} Else {
		...
		Try {
			...
			Write-Host "Could not move file to $BackupFolderPath"
		}
	}
}
...
Foreach ($group in $XMLGroups) {
	...
	If (
		...
		#Put all other groups into a list to be sorted.
		$GroupList.Add($group)
	}
}
